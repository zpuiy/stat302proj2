...
```{r}
# Store the fitted value of the linear regression data with my_gapminder data, 
#  using the life expectancy data as response variable, gdp per capita and 
#  continent data as explanatory variables.
...
for (i in 1:10) {
  pred <- my_knn_cv(input_penguins, mod_penguins$species, i, 5)$class
  err_table[i, 1] <- my_knn_cv(input_penguins, mod_penguins$species, i, 5)$cv_err
  err_table[i, 2] <- sum(as.numeric(pred != mod_penguins$species)) / 
    nrow(mod_penguins)
}
...
I would choose the 1-nearest neighbor model in practice because it has the lowest CV misclassification error rate and lower training error rate.
